...
quant_file_handle = open(project_root+"data/samples/"+"quants_emb.txt",'r')
...
outFile = open("diffScript_emb.sh",'w')
...
for strain,bg in background.iteritems():
	...
	print >>outFile, "#%s\n%s %s %s %s %s\n" % (strain,cuffdiff_cmd,strain,"Embryonic",KOSamples,WTSamples)