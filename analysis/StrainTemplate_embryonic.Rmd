...
```{r print all the splicing csPie tables}
...
```{r diff.TSS,eval=FALSE,echo=FALSE}
...
```{r diff.promoters,eval=FALSE,echo=FALSE}
...
```{r diff.CDS,eval=FALSE, echo=FALSE}
...
heatmap.2(x, trace="none", margins=c(5,30),col=InputCols,labRow=labels,dendrogram="both",lhei = c(0.1,0.90))
dev.off()
```
