...
```{r plot ALL OF THE SIGNIFICANT GENES, fig.height=20 + 1*(length(sig)), fig.width=20}
...
# how do we rescale these plots? or just axes?
```{r print all the splicing csPie plots, fig.height=20, fig.width=20}
...
```{r print GSEA biocarta, fig.height=30, fig.width=10}
...
heatmap.2(x, trace="none", margins=c(5,30), col=InputCols, labRow=labels,dendrogram="both",lhei =c(0.9,0.1),key=FALSE)
#dev.off()
```
...
heatmap.2(x, trace="none", margins=c(5,30),col=InputCols,dendrogram="both",lhei = c(0.9,0.10), key=FALSE)
#dev.off()
```
