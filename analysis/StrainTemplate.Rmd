...
```{r plot ALL OF THE SIGNIFICANT GENES, eval=FALSE, fig.height=(20 + (1*(length(sig)))), fig.width=(20 + (1*length(sig)))}
...
```{r gsea helper functions, eval=FALSE}
...
```{r print GSEA biocarta, eval=FALSE, fig.height=20, fig.width=10}
...
```{r print GSEA reactome, eval=FALSE, fig.width=10, fig.height=20}
...
```{r GO analysis setup and functions,echo=FALSE,eval=FALSE}
...
```{r GO enrichments, eval=FALSE, echo=FALSE}
...
```{r GO figures, echo=FALSE, eval=FALSE,  fig.height=5, fig.width=8}
