...
expressionBarplot(genotypingGenes,replicates=T)+theme_bw()
```

Expression heatmap:

```{r digital_geno_heatmap, fig.height=4, fig.width=8}
csHeatmap(genotypingGenes,replicates=T)+theme_bw()
```
...
from<-as.numeric(start_and_end[[1]])-windowMargin
to<-as.numeric(start_and_end[[2]])+windowMargin
...
doPlot<-function(genome=genome,name,myChr,from,to,window,bamFiles,bamNames,koStart,koWidth,koChr){
  ...
  trackSizes<-c(2,2,4,rep(1,length(bamTracks)),1)
...
```{r sigExpression, fig.height=10, fig.width=8}
...
```{r highly_expressed_sig, fig.height=8, fig.width=8}
...
```{r sigGenes logfoldchange_heatmap, fig.height=8, fig.width=10}
csFoldChangeHeatmap(sigGenes,"WT",replicates=TRUE,replicate_num=0,clustering="both")+theme_bw()
```
...
if(length(isoformSigIDs)>0){
  csHeatmap(isoformSigGenes,cluster='both', replicates=T)+theme_bw()
  } else{print("no sig isoforms")}
```
...
if(length(isoformSigIDs)>0){
  csFoldChangeHeatmap(isoforms(isoformSigGenes),"WT",replicates=TRUE,replicate_num=0,clustering="none")+theme_bw()
}else{print("no sig isoforms")}
```
...
csHeatmap(isoforms(splicingSigGenes),cluster='both',method=dist)+theme_bw()
#dev.off()
```
...
csHeatmap(splicingSigGenes,cluster='both',method=dist)+theme_bw()
```
...
csHeatmap(tssSigGenes,cluster='row',method=dist)+theme_bw()
...
csHeatmap(cdsSigGenes,cluster='row',method=dist)+theme_bw()
dev.off()

pdf("sig_relCDS_heatmap.pdf",width=10,height=20)
csHeatmap(relcdsSigGenes,cluster='row',method=dist)+theme_bw()
dev.off()
```
...
InputCols<-maPalette(low="blue",mid="white",high="red",k=100)
...
```{r print_GSEA_reactome, fig.width=8, fig.height=8}
...
```{r gsea_zscore_reactome, fig.height=8, fig.width=8}
