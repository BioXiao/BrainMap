...
if(length(isoformSigIDs)>0){
  csFoldChangeHeatmap(isoforms(isoformSigGenes),"WT",replicates=TRUE,replicate_num=0,clustering="both")+theme_bw()
}else{print("no sig isoforms")}
```
...
k <- 100
myColors<-maPalette(low=“steelblue”,mid=“white”,high=“red”,k=k)
#myBreaks<-seq(-max(abs(x)),max(abs(x)),k+1)
myBreaks<-seq(-2,2,k+1)
