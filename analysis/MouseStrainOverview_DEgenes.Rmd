...
ggplot(linc_LZ_correlation, aes(EndogenousLinc,LZ))+geom_point()+geom_text(aes(label=paste(name,timepoint,sep="_")))+xlab("Endogenous lincRNA expression log10(FPKM)")+ylab("LacZ expression log10(FPKM)")+scale_x_log10(limits=c(-10,80))+scale_y_log10(limits=c(-10,40))+geom_smooth(method="lm",se=FALSE)+geom_abline(intercept=0,slope=1)

ggsave("linc_LZ_correlation.pdf",plot=linc_LZ)

ggplot(linc_LZ_correlation, aes(EndogenousLinc,ActinInKOs,color=timepoint))+geom_point()+geom_text(aes(label=name))+geom_abline(intercept=0,slope=1)+xlab("Endogenous lincRNA expression log10(FPKM)")+ylab("Actin expression log10(FPKM)")+geom_abline(intercept=0,slope=1)+coord_trans(x="log10",y="log10",limy=c(-100,800),limx=c(-100,800))

scale_x_log10(limits=c(-100,800))+scale_y_log10(limits=c(-100,800))
+
