...
strain<-"linc-Cox2"
timepoint<-"Adult"
filename<-"linc-Cox2_vs_WT_Adult"
...
csHeatmap(cdsSigGenes,cluster='row',method=dist,labRow=FALSE)+theme_bw()
dev.off()

pdf("sig_relCDS_heatmap.pdf",width=10,height=20)
csHeatmap(relcdsSigGenes,cluster='row',method=dist,labRow=FALSE)+theme_bw()
dev.off()
```
...
ggplot(data,aes(start,test_stat,color=sig, label=gene_name))+geom_point()+scale_color_manual(values=c("black", "red"))+coord_cartesian(xlim=c(-windowSize/2, windowSize/2), ylim=c(-(max(abs(data$test_stat)+1)),(max(abs(data$test_stat)+1)))+labs(title=strain)+
                                                                                                                                         geom_text(data=subset(data, sig=='yes'))+theme_bw()+geom_vline(xintercept=0, color="blue")+geom_hline(yintercept=0,color="blue")
```
